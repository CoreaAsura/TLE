...
TLE_JSON_URL = (
    ...
    f"epoch>%3D{start_date_str}/orderby/NORAD_CAT_ID asc/format/json"
)

st.set_page_config(page_title="TLE Downloader", layout="centered")
st.title("🌌 최근 30일 TLE 다운로드 (위성 이름 포함)")
    ...
    if login_response.status_code == 200:
        try:
            ...
            if "application/json" in tle_response.headers.get("Content-Type", ""):
                ...
                if not tle_data:
                    st.warning("🚫 최근 30일간 TLE 데이터가 없습니다.")
                else:
                    formatted_tle = ""
                    for sat in tle_data:
                    ...
                    st.download_button(
                        ...
                        data=formatted_tle,
                        file_name="named_tle_recent.txt",
                        mime="text/plain"
                    )
            else:
